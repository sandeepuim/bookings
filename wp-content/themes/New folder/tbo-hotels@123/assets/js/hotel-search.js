...
jQuery(document).ready(function($) {
    ...
    function loadCountries() {
        ...
        // Try the standard AJAX call first
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success && response.data) {
                    ...
                    // First fallback - try direct endpoint
                    tryFallbackCountries();
                }
            },
            error: function() {
                console.log('Error loading countries via AJAX. Trying fallback...');
        ...
        // Fallback function to load countries if the AJAX call fails
        function tryFallbackCountries() {
            console.log('Attempting fallback country loading...');
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success && response.data) {
                        console.log('Fallback countries loaded successfully');
                        searchState.countries = response.data;
                        populateCountrySelect(response.data);
                    } else {
                        // Final fallback - hardcoded India option
                        console.log('Using hardcoded fallback for countries');
                        ...
                        searchState.countries = fallbackCountries;
    ...
    function loadCities(countryCode) {
        ...
        $.ajax({
            ...
            error: function() {
                console.log('Error loading cities via AJAX. Trying fallback...');
                // Try fallback approach
                tryFallbackCities(countryCode);
            },
            complete: function() {
        ...
        // Fallback function to load cities if the AJAX call fails
        function tryFallbackCities(countryCode) {
            console.log('Attempting fallback city loading...');
            
            // For India (IN), provide common cities as fallback
            if (countryCode === 'IN') {
                console.log('Using hardcoded fallback for India cities');
                var fallbackCities = [
                ...
                searchState.cities = fallbackCities;
            ...
            if (countryCode === 'US') {
                console.log('Using hardcoded fallback for US cities');
                var fallbackCities = [
            ...
            // For other countries, show a message
            showError($citySelect, 'Unable to load cities. Using direct search.');
            $citySelect.html('<option value="0">Direct Search - Enter hotel name</option>');
        }
    }
    ...
    function buildHotelCard(hotel) {
        ...
        // Choose room button - using multiple methods for redundancy
        html += '<form action="/bookings/room-redirect.php" method="get" class="room-selection-form" style="display:inline;">';
        html += '<input type="hidden" name="hotel_code" value="' + hotelCode + '">';
        
        // Add all current search parameters as hidden inputs
        ...
        html += '<button type="submit" class="choose-room-btn" data-hotel-code="' + hotelCode + '" onclick="return redirectToRoomSelection(\'' + hotelCode + '\');">Choose Room</button>';
        html += '</form>';
    ...
    // Direct function to handle room selection redirect
    // This ensures we have a reliable way to redirect even if event binding fails
    function redirectToRoomSelection(hotelCode) {
        console.log('Direct redirect function called for hotel code:', hotelCode);
        
        // Get current search parameters
        var searchParams = new URLSearchParams(window.location.search);
        
        // Create a URL to our PHP redirect script (more reliable approach)
        var roomsUrl = '/bookings/room-redirect.php?hotel_code=' + hotelCode;
        ...
        console.log('Redirecting to room selection URL via redirect script:', roomsUrl);
        
        // Redirect to the room redirect script
        window.location.href = roomsUrl;
        
        // Prevent default behavior if used in an onclick
        return false;
    }
