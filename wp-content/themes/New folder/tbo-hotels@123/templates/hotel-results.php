...
jQuery(document).ready(function($) {
    ...
    }, 500); // Reduced timeout for faster initial load
    ...
    function performHotelSearch(countryCode, cityCode, checkIn, checkOut, adults, children, rooms) {
        ...
        // Show enhanced loading animation with progress updates
        $('#hotel-results').html(`
            <div class="loading-container">
                <div class="loading-spinner"></div>
                <h3>Searching Hotels...</h3>
                <div class="loading-progress">
                    ...
                    <div class="progress-text">Connecting to hotel database...</div>
                </div>
            </div>
        `);
        
        // Update progress indicator to show activity
        var progressInterval = setInterval(function() {
            var currentWidth = parseInt($('.progress-fill').css('width'));
            var containerWidth = $('.progress-bar').width();
            var percentage = Math.min(90, (currentWidth / containerWidth * 100) + 10);
            
            $('.progress-fill').css('width', percentage + '%');
            
            // Update status message
            if (percentage < 30) {
                $('.progress-text').text('Connecting to hotel database...');
            } else if (percentage < 60) {
                $('.progress-text').text('Fetching hotel availability...');
            } else {
                $('.progress-text').text('Processing results...');
            }
        }, 800);
    ...
    function displayHotelResults(hotels) {
        ...
        // Create results container with total count
        let resultsHeader = `
            <div class="results-summary">
                <h3>${hotels.length} Hotels Found</h3>
                <p>Showing fastest results first. More properties loading...</p>
            </div>
        `;
        
        // Initialize with empty results
        $('#hotel-results').html(resultsHeader + '<div id="hotel-cards-container"></div>');
        
        // Load hotels progressively in batches for better performance
        const batchSize = 5;
        let currentIndex = 0;
        
        function loadNextBatch() {
            if (currentIndex >= hotels.length) {
                // All hotels loaded, update summary
                $('.results-summary p').text(`${hotels.length} properties in ${hotels[0].CityName || 'this destination'}`);
                return;
            }
            
            // Get the next batch
            const batch = hotels.slice(currentIndex, currentIndex + batchSize);
            
            // Create HTML for this batch
            let batchHtml = '';
            batch.forEach(function(hotel) {
                batchHtml += buildHotelCard(hotel);
            });
            
            // Append to container
            $('#hotel-cards-container').append(batchHtml);
            
            // Force horizontal layout on newly added cards
            forceHorizontalLayout();
            
            // Update progress in summary
            $('.results-summary p').text(`Loaded ${Math.min(currentIndex + batchSize, hotels.length)} of ${hotels.length} properties...`);
    ...
    function buildHotelCard(hotel) {
        const originalPrice = Math.floor(Math.random() * 3000) + 5000;
        const currentPrice = Math.floor(originalPrice * 0.82);
        const rating = (4.0 + Math.random() * 1.0).toFixed(1);
        const reviews = Math.floor(Math.random() * 3000) + 1000;
                ...
                <div class="hotel-details-section">
                    <div>
                        ...
                        <div style="display: flex; margin-bottom: 12px;">
                            ${generateStars(hotel.HotelRating)}
                        </div>
                        <p style="color: #666; font-size: 14px; margin-bottom: 16px;">📍 ${hotel.Address || 'Address information not available'}</p>
                ...
                <div class="hotel-pricing-section">
                    ...
                    <div>
                        <div style="margin-bottom: 20px;">
                            ...
                            <div style="text-decoration: line-through; color: #999; font-size: 14px; margin-bottom: 4px;">₹${originalPrice.toLocaleString()}</div>
                            <div style="font-size: 24px; font-weight: 700; color: #333; margin-bottom: 4px;">₹ ${currentPrice.toLocaleString()}</div>
                            <div style="font-size: 11px; color: #666;">+₹200 taxes & fees<br>per room per night</div>
                        </div>
                        
                        <button style="background: #ff6b35; color: white; border: none; padding: 12px 24px; border-radius: 6px; font-size: 14px; font-weight: 600; cursor: pointer; width: 100%;">
                            Choose Room
                        </button>
                    </div>
                </div>
