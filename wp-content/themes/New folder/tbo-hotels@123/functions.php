...
// Include PHP leak fix
require_once(TBO_HOTELS_DIR . '/includes/php-leak-fix.php');

// Include optimizations
require_once(TBO_HOTELS_DIR . '/includes/tbo-hotels-optimization.php');

// Direct error fix - highest priority to catch syntax errors
function tbo_hotels_direct_error_fix() {
    // Output direct fix in the head for fastest execution
    ?>
    <script>
    // TBO Hotels Urgent Error Fix - Runs inline for immediate effect
    (function() {
        console.log('TBO Hotels Urgent Fix: Active');
        
        // Install global error handler immediately
        window.addEventListener('error', function(event) {
            if (event && event.message) {
                // Specifically target the errors shown in the console
                if (event.message.includes('Unexpected token \'catch\'') || 
                    event.message.includes('missing ) after argument list')) {
                    console.warn('TBO Hotels Urgent Fix: Caught error - ' + event.message);
        ...
        // Add specific pattern fixes for the errors in hotel-results
        function patchHotelResultsPage() {
            if (window.location.href.includes('hotel-results') || 
                window.location.href.includes('check_in=')) {
                
                // Add a MutationObserver to catch and fix scripts as they're added
                var observer = new MutationObserver(function(mutations) {
                    mutations.forEach(function(mutation) {
                        if (mutation.type === 'childList') {
                            mutation.addedNodes.forEach(function(node) {
                                // Only process script nodes
        ...
        // Run immediately
        patchHotelResultsPage();
    })();
    </script>
    <?php
}
add_action('wp_head', 'tbo_hotels_direct_error_fix', 1); // Priority 1 ensures it runs very early

// Enqueue all error fix scripts
function tbo_hotels_enqueue_error_fix() {
    // Original console error fix
    wp_enqueue_script(
        'tbo-hotels-console-error-fix',
        TBO_HOTELS_URI . '/includes/console-error-fix.js',
        array(),
        TBO_HOTELS_VERSION,
        true
    );
    
    // Direct error fix script - higher priority
    wp_enqueue_script(
        'tbo-hotels-direct-error-fix',
        TBO_HOTELS_URI . '/assets/js/direct-error-fix.js',
        array('jquery'),
        TBO_HOTELS_VERSION,
        false // Load in header for earlier execution
    );
}
add_action('wp_enqueue_scripts', 'tbo_hotels_enqueue_error_fix', 5); // Priority 5 ensures it loads early
...
// Include Room API functions
require_once(TBO_HOTELS_DIR . '/includes/tbo-hotels-room-api.php');

// Include Room Selection Button Fix
require_once(TBO_HOTELS_DIR . '/includes/room-button-fix.php');

// Include our new files - using the fixed version (after the existing API)
include_once(TBO_HOTELS_DIR . '/../twentytwentyone/tbo-room-functions-fixed.php');
require_once(TBO_HOTELS_DIR . '/../twentytwentyone/direct-button-fix.php');
require_once(TBO_HOTELS_DIR . '/../twentytwentyone/hotel-button-enhancement.php');
require_once(TBO_HOTELS_DIR . '/../twentytwentyone/button-fix-direct-injection.php');

// Enqueue room selection styles and scripts
function tbo_hotels_enqueue_room_selection_styles() {
    wp_enqueue_style('tbo-room-selection', get_template_directory_uri() . '/../twentytwentyone/room-selection.css', array(), TBO_HOTELS_VERSION);
    
    // Add our button fix JavaScript - using the advanced version
    wp_enqueue_script('tbo-advanced-button-fix', get_template_directory_uri() . '/../twentytwentyone/advanced-button-fix.js', array(), TBO_HOTELS_VERSION, true);
    
    // Add our dropdown fix JavaScript
    wp_enqueue_script('tbo-dropdown-fix', get_template_directory_uri() . '/assets/js/dropdown-fix.js', array('jquery'), TBO_HOTELS_VERSION, true);
...
add_action('wp_enqueue_scripts', 'tbo_hotels_enqueue_room_selection_styles');
...
function tbo_hotels_api_request($endpoint, $data = array(), $method = 'GET') {
    ...
    // Check for fallback mode - if API is down, use cached data or sample data
    $fallback_mode = get_option('tbo_hotels_api_fallback_mode', false);
    
    // Get from API cache first if in fallback mode (only for GET requests or list endpoints)
    if ($fallback_mode && ($method === 'GET' || strpos($endpoint, 'List') !== false)) {
        $cache_key = 'tbo_api_' . $endpoint . '_' . md5(serialize($data));
    ...
    if ($endpoint === 'Search') {
        // Reduce timeout for search since we've reduced hotel count
        $timeout = 25;
    } elseif ($endpoint === 'TBOHotelCodeList' || $endpoint === 'CountryList' || $endpoint === 'CityList') {
        $timeout = 15; // Less timeout for listing endpoints
    }
    ...
    // Handle errors and set fallback mode if needed
    if (is_wp_error($response)) {
        error_log('TBO API Error: ' . $response->get_error_message());
        
        // Set fallback mode if API is down
        update_option('tbo_hotels_api_fallback_mode', true);
        update_option('tbo_hotels_api_last_error', 'Connection error: ' . $response->get_error_message());
        update_option('tbo_hotels_api_last_error_time', time());
    ...
    if ($response_code !== 200) {
        ...
        // Set fallback mode if API returns error status
        if ($response_code >= 500) {
            update_option('tbo_hotels_api_fallback_mode', true);
            update_option('tbo_hotels_api_last_error', 'Server error: ' . $response_code);
            update_option('tbo_hotels_api_last_error_time', time());
        }
    ...
    // If we got here, API is working - reset fallback mode
    if (get_option('tbo_hotels_api_fallback_mode', false)) {
        update_option('tbo_hotels_api_fallback_mode', false);
    }
    
    // Cache the response for future use in fallback mode
    if ($method === 'GET' || strpos($endpoint, 'List') !== false) {
        $cache_key = 'tbo_api_' . $endpoint . '_' . md5(serialize($data));
...
function tbo_hotels_get_hotel_details($city_code) {
    ...
    // Check cache first
    $cache_key = 'tbo_hotels_details_' . $city_code;
    $hotel_details = get_transient($cache_key);
    
    if ($hotel_details !== false) {
    ...
    if (isset($response['Hotels']) && is_array($response['Hotels'])) {
        ...
        // Cache for 6 hours
        set_transient($cache_key, $hotel_details, 6 * HOUR_IN_SECONDS);
    }
...
function tbo_hotels_search_hotels($params) {
    ...
    // Try to get from cache first (use a hash of search params as key)
    $cache_key = 'tbo_search_' . md5(serialize($params));
    ...
    // Limit to 50 hotel codes to improve performance
    $hotel_codes = array_slice($hotel_codes, 0, 50);
    ...
    if (isset($response['HotelResult']) && is_array($response['HotelResult'])) {
        ...
        set_transient($cache_key, $enhanced_results, 1 * HOUR_IN_SECONDS);
...
function tbo_hotels_scripts() {
    ...
    // Enqueue hotel rooms styles
    wp_enqueue_style('tbo-hotels-rooms', TBO_HOTELS_URI . '/assets/css/hotel-rooms.css', array(), TBO_HOTELS_VERSION);
    ...
    // Enqueue hotel rooms script
    wp_enqueue_script('tbo-hotels-rooms', TBO_HOTELS_URI . '/assets/js/hotel-rooms.js', array('jquery'), TBO_HOTELS_VERSION, true);
    
    // Localize script with AJAX URL and site URLs
    wp_localize_script('tbo-hotels-search', 'tbo_hotels_params', array(
        ...
        'site_url' => site_url(),
        'hotel_rooms_url' => get_permalink(get_page_by_path('hotel-rooms'))
    ));
}
...
/**
 * Fix for the hotel rooms URL - Special rewrite rule
 */
function tbo_hotels_rewrite_rules() {
    add_rewrite_rule(
        'hotel-rooms/([^/]+)/?$',
        'index.php?pagename=hotel-rooms&hotel_code=$matches[1]',
        'top'
    );
}
add_action('init', 'tbo_hotels_rewrite_rules');

/**
 * Flush rewrite rules on theme activation
 */
function tbo_hotels_activation() {
    tbo_hotels_rewrite_rules();
    flush_rewrite_rules();
}
register_activation_hook(__FILE__, 'tbo_hotels_activation');

/**
 * Add hotel_code as a query var that WordPress recognizes
 */
function tbo_hotels_query_vars($vars) {
    $vars[] = 'hotel_code';
    return $vars;
}
add_filter('query_vars', 'tbo_hotels_query_vars');
