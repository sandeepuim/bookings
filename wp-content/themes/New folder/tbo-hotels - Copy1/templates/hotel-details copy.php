...
function tbo_hotels_get_hotel_details_from_search_api($params) {
    ...
    $payload = json_encode([
        ...
        "GuestNationality" => $params['country_code'],
        "PaxRooms" => [["Adults" => $params['adults'], "Children" => $params['children']]],
        "ResponseTime" => 20,
                                        ...
                                        <div style="flex: 1; background: #fff; border: 1px solid #eee; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.04); padding: 24px; min-width: 320px;">
                                            ...
                                            <?php if ($firstRoom): ?>
                                                ...
                                                <div style="display: flex; gap: 12px; margin-top: 18px;">
                                                    ...
                                                    <form action="<?php echo esc_url(home_url('/hotel-booking/')); ?>" method="get" style="margin: 0;">
                ...
                <div class="hotel-details-main">
                    ...
                    <?php if (!empty($hotel_data['RoomDetails']) && is_array($hotel_data['RoomDetails'])): ?>
                        <section class="hotel-room-types yatra-style">
                            ...
                            <!-- Featured Room (first) -->
                            <div class="featured-room-card yatra-card">
                                <div class="room-type-image-wrap">
                                    <?php if (!empty($firstRoom['RoomImages'][0])): ?>
                                        <img class="room-type-image" src="<?php echo esc_url($firstRoom['RoomImages'][0]); ?>" alt="<?php echo esc_attr($firstRoom['RoomTypeName'] ?? 'Room'); ?>">
                                    <?php else: ?>
                                        <img class="room-type-image" src="<?php echo esc_url(get_template_directory_uri() . '/assets/img/room-placeholder.jpg'); ?>" alt="Room">
                                    <?php endif; ?>
                                </div>
                                <div class="room-type-info">
                                    <h3 class="room-type-name yatra-room-title"><?php echo esc_html($firstRoom['RoomTypeName'] ?? 'Room'); ?></h3>
                                    <div class="room-type-occupancy yatra-occupancy">
                                        <span title="Adults"><i class="fas fa-user-friends"></i> <?php echo intval($firstRoom['MaxAdults'] ?? 2); ?> Adults</span>
                                        <?php if (isset($firstRoom['MaxChildren'])): ?>
                                            <span title="Children"><i class="fas fa-child"></i> <?php echo intval($firstRoom['MaxChildren']); ?> Children</span>
                                        <?php endif; ?>
                                    </div>
                                    <?php if (!empty($firstRoom['RoomFacilities'])): ?>
                                        <div class="room-type-amenities yatra-amenities">
                                            <?php foreach (array_slice($firstRoom['RoomFacilities'], 0, 6) as $facility): ?>
                                                <span class="room-amenity yatra-amenity"><i class="fas fa-check-circle"></i> <?php echo esc_html($facility); ?></span>
                                            <?php endforeach; ?>
                                        </div>
                                    <?php endif; ?>
                                    <div class="room-type-price yatra-price">
                                    ...
                                    <?php if (!empty($firstRoom['CancellationPolicy'])): ?>
                                        <div class="room-type-cancellation yatra-cancellation">
                                            <i class="fas fa-undo-alt"></i> <span class="cancellation-label">Free Cancellation:</span> <span class="cancellation-value"><?php echo esc_html($firstRoom['CancellationPolicy']); ?></span>
                                        </div>
                                    <?php endif; ?>
                                </div>
                                <div class="room-type-action yatra-action">
                                    <form action="<?php echo esc_url(home_url('/hotel-booking/')); ?>" method="get">
                                        <input type="hidden" name="hotel_code" value="<?php echo esc_attr($hotel_code); ?>">
                                        <input type="hidden" name="city_code" value="<?php echo esc_attr($city_code); ?>">
                                        <input type="hidden" name="checkin" value="<?php echo esc_attr($checkin); ?>">
                                        <input type="hidden" name="checkout" value="<?php echo esc_attr($checkout); ?>">
                                        ...
                                        <input type="hidden" name="rooms" value="<?php echo esc_attr($rooms); ?>">
                                        <input type="hidden" name="room_type" value="<?php echo esc_attr($firstRoom['RoomTypeName'] ?? ''); ?>">
                                        <button type="submit" class="select-room-button yatra-book-btn">Book Now</button>
                                    </form>
                                </div>
                            </div>
                            <!-- Other Rooms -->
                    ...
                    <div class="booking-widget">
                        ...
                        <div class="price-summary">
                            ...
                            <div class="current-price">
                                <?php
                                $min_price = isset($hotel_data['MinHotelPrice']) ? $hotel_data['MinHotelPrice'] : null;
                                $currency_code = (isset($hotel_data['Price']) && isset($hotel_data['Price']['CurrencyCode'])) ? $hotel_data['Price']['CurrencyCode'] : 'INR';
                                if ($min_price !== null) {
                                    echo tbo_hotels_format_price($min_price, $currency_code);
                                } else {
                                    echo 'Price not available';
                                }
                                ?>
