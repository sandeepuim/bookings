...
function tbo_hotels_get_hotel_codes($city_code) {
    ...
    if (isset($response['Hotels']) && is_array($response['Hotels'])) {
        ...
        error_log('Hotel Codes for city ' . $city_code . ': ' . print_r($hotel_codes, true));
        return $hotel_codes;
    }
...
function tbo_hotels_get_hotel_details($city_code) {
    ...
    error_log('Hotel Details for city ' . $city_code . ': ' . print_r($hotel_details, true));
...
error_log('Hotel Codes sent to Search API: ' . $hotel_codes_string);
...
//echo "<h3>➡️ Request Body (Payload):</h3><pre>" . print_r($search_data, true) . "</pre>";
